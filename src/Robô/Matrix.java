...
public class Matrix {
	...
	// vetor de móveis
    private final int[][] occupied = new int[10][3];
	...
	public Matrix( int lines, int columns){
		...
		for( int line = BORDER_TOP; line <= this.border_bottom; line++ )
			for( int column = BORDER_LEFT; column <= this.border_right; column++ )
				this.matrix[ line ][ column ] = this.EMPTY;		
		// desenha as paredes superior e inferior 
		for( int column = BORDER_LEFT -1; column <= this.border_right +1; column++ ){
			this.matrix[BORDER_TOP -1][column] = '-';
			this.matrix[this.border_bottom +1][column] = '-';
		}
		// desenha as paredes esquerda e direita
		for( int line = BORDER_TOP -1; line <= this.border_bottom +1; line++ ){
			this.matrix[line][BORDER_LEFT -1] = '|';
